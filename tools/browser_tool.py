...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, str]:
    ...
    if cdp_override and not force_local:
        with _cleanup_lock:
            # Check and insert the live-CDP session under one lock. Otherwise two
            # simultaneous browser tasks can both observe no active live-CDP task
            # and create overlapping wrappers around the same shared Chrome.
...
def _build_agent_browser_env(task_socket_dir: str) -> Dict[str, str]:
    ...
    # Tell the agent-browser daemon to self-terminate after being idle for our
    # configured inactivity timeout. This preserves the daemon-side counterpart
    # to Python-side _cleanup_inactive_browser_sessions after the live-CDP
    # persistent-session refactor centralized browser CLI env creation here.
...
def _run_browser_command(
    ...
    Args:
        ...
        _live_cdp_retry_attempted: Internal flag to prevent infinite retry loops.
    ...
    try:
        # _execute_browser_cli preserves the hardening from the old inline path:
        # empty stdout for non-empty-ok commands is a failure with
        # "returned no output" instead of being silently treated as success.
        result = _execute_browser_cli(
            cmd_parts,
            command,
            timeout,
