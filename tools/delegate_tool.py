    ...
    **_kwargs,
    ...
    def _finalize_execution_receipt(
        *,
        ...
        """Best-effort receipt finalization for delegated child execution."""
        nonlocal _receipt_finalized
        if _receipt is None or _receipt_finalized:
            return
        try:
            from tools.execution_receipts import finalize_receipt
            ...
            _receipt_finalized = True
        except Exception as exc:
            logger.debug("Failed to finalize delegation execution receipt: %s", exc)
    ...
    try:
        ...
        try:
            ...
            _receipt = create_receipt(
                ...
                metadata={
                    "goal": goal[:200] if isinstance(goal, str) else "",
                    "task_index": task_index,
                    "subagent_id": _subagent_id or "",
                },
            )
        except Exception as exc:
            logger.debug("Failed to create delegation execution receipt: %s", exc)
        ...
        _finalize_execution_receipt(
            status="cancelled" if status == "interrupted" else status,
            duration_seconds=duration,
